...
train_data = train_data.shuffle(240) # if you want to shuffle your data
train_data = train_data.batch(batch_size)

# create testing Dataset and batch it
test_data = tf.data.Dataset.from_tensor_slices(test)
test_data = test_data.shuffle(240)
...
w = tf.get_variable("w", shape=(12544,L), initializer=tf.random_normal_initializer(0,0.01))
...
w5 = tf.get_variable("w5", shape=(L,40), initializer=tf.random_normal_initializer(0,0.01))
b5 = tf.get_variable("b5", shape=(1,40), initializer=tf.zeros_initializer())
...
with tf.Session() as sess:
    ...
    total_cmat = np.zeros((40,40),dtype="int32")
    try:
        while True:
...
#writer.close()
#Y = list(range(n_epochs))
#plt.plot(loss_vector,Y)